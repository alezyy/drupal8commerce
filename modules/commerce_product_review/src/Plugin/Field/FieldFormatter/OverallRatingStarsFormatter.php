...
class OverallRatingStarsFormatter extends FormatterBase implements ContainerFactoryPluginInterface, OverallRatingEmptyTextFormatterInterface {
   ...
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, AccountProxyInterface $current_user, EntityTypeManagerInterface $entity_type_manager, NumberFormatterFactoryInterface $number_formatter_factory) {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      $rating = $this->numberFormatter->format($item->score);
      $elements[$delta] = [
        '#type' => 'container',
        'overall_rating' => [
          '#type' => 'html_tag',
      ...
      if ($this->getSetting('show_overview_link')) {
        $elements[$delta]['overview_link'] = [
          '#type' => 'link',
          '#title' => $this->formatPlural(count($existing_reviews), 'View @count review', 'View all @count review(s)'),
