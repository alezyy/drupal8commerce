...
class MultiplePaymentsCheckoutTest extends CommerceBrowserTestBase {
  ...
  public function testSuccessfulCheckout() {
    ...
    // @todo: free order handling is supposed to be handled without a payment gateway in future, per Slack chats
    // But for now, we'll use a "free" manual payment gateway in here that creates $0 pending payments.
    $this->assertSession()->pageTextContains('Free order');
    ...
    foreach ($payments as $payment) {
      ...
      switch ($payment->getPaymentGatewayId()) {
        ...
        case 'store_credit':
          ...
          break;
        case 'free':
          $found_payment_gateways['free'] = 'free';
          $this->assertEquals(0, $payment->getAmount()->getNumber());
          $this->assertEquals('USD', $payment->getAmount()->getCurrencyCode());
          $this->assertEquals('pending', $payment->getState()->getString());
          break;
      }
    }
    $this->assertEquals(3, count($found_payment_gateways));
